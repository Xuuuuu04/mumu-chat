...
class SerpSearchService(
    ...
    suspend fun search(
        ...
        var downgradedFrom: String? = null
        for (provider in providersInOrder) {
            try {
                val r = provider.search(q, limit, page)
                if (r.results.isEmpty()) throw SerpProviderException("empty", "未获取到搜索结果")
                val normalized = if (r.engine == provider.id) r else r.copy(engine = provider.id)
                return if (downgradedFrom.isNullOrBlank()) {
                    normalized
                } else {
                    normalized.copy(downgradedFrom = downgradedFrom)
                }
            } catch (e: SerpProviderException) {
                lastError = e
                if (downgradedFrom == null) downgradedFrom = provider.id
