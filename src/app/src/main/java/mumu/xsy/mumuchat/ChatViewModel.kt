...
import mumu.xsy.mumuchat.tools.search.SearxngInstances
...
- 严格避免编造：不确定就调用工具或明确“不确定/需要验证”。"""
    ...
    var inputFocusNonce by mutableStateOf(0)
    ...
    suspend fun probeSearxngDetail(baseUrl: String): SearxngProbeResult {
        val base = baseUrl.trim().trimEnd('/')
        if (base.isBlank()) return SearxngProbeResult(ok = false, baseUrl = baseUrl, code = "invalid_config", message = "地址为空")
        val url = (base + "/search").toHttpUrlOrNull()
            ?.newBuilder()
            ?.addQueryParameter("q", "MuMuChat")
            ?.addQueryParameter("format", "json")
            ?.addQueryParameter("pageno", "1")
            ?.addQueryParameter("categories", "general")
            ?.build()
            ?.toString()
            ?: return SearxngProbeResult(ok = false, baseUrl = baseUrl, code = "invalid_config", message = "地址无效")
        ...
        val r = withContext(Dispatchers.IO) { remoteApiClient.fetch(endpoint, bypassCache = true, cacheKey = "searxng_probe|$base") }
        if (!r.ok) return SearxngProbeResult(ok = false, baseUrl = base, code = r.code, message = r.message)
        val root = runCatching { JsonParser.parseString(r.body.orEmpty()) }.getOrNull()
            ?: return SearxngProbeResult(ok = false, baseUrl = base, code = "parse_failed", message = "返回不是有效 JSON")
        if (!root.isJsonObject) return SearxngProbeResult(ok = false, baseUrl = base, code = "parse_failed", message = "返回不是 JSON 对象")
        val obj = root.asJsonObject
        if (!obj.has("results")) return SearxngProbeResult(ok = false, baseUrl = base, code = "parse_failed", message = "缺少 results 字段")
        return SearxngProbeResult(ok = true, baseUrl = base)
    }

    suspend fun probeSearxng(baseUrl: String): Boolean = probeSearxngDetail(baseUrl).ok

    suspend fun autoPickSearxng(): Pair<String?, List<SearxngProbeResult>> {
        val results = mutableListOf<SearxngProbeResult>()
        for (c in SearxngInstances.candidates) {
            val r = probeSearxngDetail(c)
            results.add(r)
            if (r.ok) return r.baseUrl to results
        }
