...
object ToolsCatalog {
    fun getToolsDefinition() = JsonArray().apply {
        ...
        add(createTool("tavily_search", "AI-native 搜索（结构化结果，优先用于联网检索）。", mapOf(
            "query" to "string",
            "max_results" to "integer",
            "search_depth" to "string"
        ), required = setOf("query")))
        add(createTool("browse_url", "阅读网页全文", mapOf("url" to "string")))
        add(createTool("serp_search", "SERP 搜索（标题/URL/snippet；支持 engine=auto/baidu/duckduckgo/searxng；searxng 需先在设置中填地址）。", mapOf(
            "query" to "string",
            "engine" to "string",
            "limit" to "integer",
            "page" to "integer",
        ...
        add(createTool("docs_add_text", "添加文档到本地资料库，用于检索与引用。", mapOf(
            "title" to "string",
            "content" to "string",
            "tags" to "string"
        ), required = setOf("content")))
        add(createTool("docs_list", "列出本地资料库文档。", mapOf("limit" to "integer")))
        add(createTool("docs_search", "检索本地资料库文档（关键字检索，返回片段）。", mapOf(
            "query" to "string",
            "limit" to "integer"
        ), required = setOf("query")))
        add(createTool("vision_ocr_url", "对图片 URL 做 OCR，提取文字。", mapOf(
            "image_url" to "string",
            "lang_hint" to "string",
            "model" to "string"
        ), required = setOf("image_url")))
        add(createTool("audio_transcribe", "转写音频文件为文字（需要选择音频文件）。", mapOf(
            "language" to "string",
            "prompt" to "string"
        )))
        add(createTool("mcp_list_servers", "列出已配置的 MCP servers。", emptyMap()))
        add(createTool("mcp_list_tools", "从指定 MCP server 拉取 tools/list。", mapOf("server_id" to "string"), required = setOf("server_id")))
        add(createTool("mcp_call", "调用 MCP server 的 tools/call。", mapOf(
            "server_id" to "string",
            "name" to "string",
            "arguments" to "object"
        ), required = setOf("server_id", "name")))
        ...
        add(createTool("notify_set_timer", "设置倒计时提醒（优先直接启动系统计时器；不支持时会打开系统计时器界面）。", mapOf(
            "seconds" to "integer",
            "message" to "string"
