...
class RemoteApiClient(
    ...
    fun fetch(endpoint: RemoteEndpoint, bypassCache: Boolean = false, cacheKey: String = endpoint.id): RemoteFetchResult {
        ...
        if (!bypassCache) {
            cache.get(cacheKey, endpoint.ttlMs)?.let { (cached, ct) ->
        ...
        return try {
            client.newCall(req).execute().use { resp ->
                ...
                if (!resp.isSuccessful) {
                    ...
                    if (!bypassCache) cache.put(cacheKey, body, ct)
                    RemoteFetchResult(
                        ok = true,
                        httpCode = http,
                        latencyMs = System.currentTimeMillis() - start,
