...
- 引用搜索来源（如果有）。"""
    ...
    private suspend fun loadSessionsFromDataStore() {
        try {
            ...
            val json = withContext(Dispatchers.IO) { context.sessionDataStore.data.first()[SESSIONS_KEY] }
            if (json.isNullOrEmpty()) return

            val loadedSessions: List<ChatSession> = gson.fromJson(
                json,
                object : com.google.gson.reflect.TypeToken<List<ChatSession>>() {}.type
            )
    ...
    fun stopGeneration() { currentEventSource?.cancel(); currentGenerationJob?.cancel(); setGenerating(false) }
