...
${settings.userPersona}
            ...
            when (name) {
                ...
                "delete_memory" -> {
                    ...
                    if (index == null || index < 0 || index >= settings.memories.size) {
                        toolErr(name, "无效的记忆索引 $index (共 ${settings.memories.size} 条)")
                    } else {
    ...
    private fun fetchHtmlWithRedirects(startUrl: String): Pair<String, String> {
        ...
        repeat(MAX_BROWSE_REDIRECTS + 1) { hop ->
            ...
            if (hop == MAX_BROWSE_REDIRECTS) break
        }
    ...
    private fun memoriesAsJson(): JsonArray {
        val arr = JsonArray()
        settings.memories.forEachIndexed { idx, text ->
    ...
    private fun upsertMemory(key: String, value: String) {
        ...
        val line = "$k: $v"
        val prefix = "$k:"
        val updated = settings.memories.toMutableList()
    ...
    private fun executeBrowseUrlSync(url: String): String = try {
        val err = validateBrowseUrl(url)
        if (err != null) return toolErr("browse_url", err)
