...
- 严格避免编造：不确定就调用工具或明确“不确定/需要验证”。"""
    ...
    fun sendMessage(context: Context, text: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val baseAttachments = cachedFiles.mapNotNull { (meta, file) ->
                val mime = meta.mimeType
                if (!mime.isNullOrBlank() && mime.startsWith("image/")) {
                    val path = saveImageToInternalStorage(context, Uri.fromFile(file))
                    if (path != null) {
                        null
                    } else {
                        ChatAttachment(
            ...
            val combinedImageUrls = finalImageUrls + prepared.extraImageUrls
