    ...
    private inner class ToolEngine {
        ...
        suspend fun execute(name: String, args: JsonObject, sIdx: Int, aiMsgIndex: Int): String {
            ...
            return try {
                ...
                record(name, ok = false, elapsedMs = SystemClock.elapsedRealtime() - start)
                toolErr(name, "工具执行错误: ${e.message}")
            }
        }

        private fun record(name: String, ok: Boolean, elapsedMs: Long) {
        ...
        private fun settingsSummaryAsJson(): JsonObject {
            return JsonObject().apply {
                ...
                addProperty("memories_count", settings.memories.orEmpty().size)
