...
- 引用搜索来源（如果有）。"""
    ...
    private inner class ToolEngine {
        ...
        init {
            ...
            register("calendar_insert_silent") { args, _, _ ->
                ...
                val eventId = withContext(Dispatchers.IO) { insertCalendarEventSilent(title, startMs, endMs, location, notes) }
                if (eventId.isNullOrBlank()) {
                    ...
                    toolOk("calendar_insert_silent", JsonObject().apply { addProperty("event_id", eventId) })
                }
            }
    ...
    private fun getWritableCalendarId(): Long? {
        ...
        cr.query(CalendarContract.Calendars.CONTENT_URI, projection, selection, null, order)?.use { c ->
            if (c.moveToFirst()) {
                ...
                if (idxId >= 0) return c.getLong(idxId)
            }
        }
        return null
    }
    ...
    private fun insertCalendarEventSilent(
        ...
        val calId = getWritableCalendarId() ?: return null
        val cr = getApplication<Application>().contentResolver
...
${settings.userPersona}
    ...
    private fun executeAudioTranscribeSync(uri: Uri, language: String?, prompt: String?): String = try {
        startKeepAlive("audio", "音频转写中", uri.toString().take(120))
        val cr = getApplication<Application>().contentResolver
        val bytes = cr.openInputStream(uri)?.use { it.readBytes() } ?: return toolErr("audio_transcribe", "读取音频失败", code = "read_failed")
        if (bytes.isEmpty()) return toolErr("audio_transcribe", "音频为空", code = "empty")
        if (bytes.size > 25 * 1024 * 1024) return toolErr("audio_transcribe", "音频过大（>25MB）", code = "too_large")
