...
- 引用搜索来源（如果有）。"""
    ...
    private inner class ToolEngine {
        ...
        init {
            ...
            register("browse_url") { args, _, _ ->
                ...
                if (url.isBlank()) toolErr("browse_url", "URL 不能为空")
                    ...
                    if (err != null) toolErr("browse_url", err) else executeBrowseUrlSync(url)
                }
            }
        ...
        suspend fun execute(name: String, args: JsonObject, sIdx: Int, aiMsgIndex: Int): String {
            ...
            return try {
                val result = handler(args, sIdx, aiMsgIndex)
...
${settings.userPersona}
    ...
    private fun validateBrowseUrl(url: String): String? {
        ...
        val deny = settings.browseDenylist.orEmpty()
            .map { it.trim().trimStart('.').lowercase(Locale.ROOT) }
            .filter { it.isNotBlank() }
        if (deny.any { d -> hostLower == d || hostLower.endsWith(".$d") }) return "命中浏览 denylist"

        val allow = settings.browseAllowlist.orEmpty()
            .map { it.trim().trimStart('.').lowercase(Locale.ROOT) }
            .filter { it.isNotBlank() }
        if (allow.isNotEmpty() && allow.none { a -> hostLower == a || hostLower.endsWith(".$a") }) return "不在浏览 allowlist"
    ...
    private fun executeJsCalculate(code: String): String {
        ...
        return try {
            ...
            val truncated = output.length > JS_MAX_OUTPUT_CHARS
            val safeOutput = if (truncated) output.take(JS_MAX_OUTPUT_CHARS) else output

            toolOk("calculate", JsonObject().apply {
                addProperty("output", safeOutput.ifBlank { "" })
                addProperty("output_length", output.length)
