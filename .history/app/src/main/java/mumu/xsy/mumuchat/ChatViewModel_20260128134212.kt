...
- 严格避免编造：不确定就调用工具或明确“不确定/需要验证”。"""
    ...
    suspend fun probeSearxng(baseUrl: String): Boolean {
        ...
        val root = runCatching { JsonParser.parseString(r.body.orEmpty()) }.getOrNull() ?: return false
        if (!root.isJsonObject) return false
        val arr = root.asJsonObject.getAsJsonArray("results") ?: return false
        return arr.size() >= 0
    }

    suspend fun autoPickSearxng(): String? {
        for (c in SearxngInstances.candidates) {
            if (probeSearxng(c)) return c.trim().trimEnd('/')
        }
        return null
    }
