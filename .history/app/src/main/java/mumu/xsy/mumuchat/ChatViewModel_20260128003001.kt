...
- 引用搜索来源（如果有）。"""
    ...
    fun addMemory(fact: String) {
        ...
        val existing = settings.memories.asSequence()
            .map { it.trim().replace("\\s+".toRegex(), " ") }
            .toSet()
        if (capped in existing) return
        val next = (settings.memories + capped).takeLast(MAX_MEMORIES)
    ...
    fun updateMemory(index: Int, text: String) {
        ...
        if (normalized.isBlank()) return
        val updated = settings.memories.toMutableList().apply { if(index in indices) this[index] = normalized.take(MAX_MEMORY_CHARS) }
        saveSettings(settings.copy(memories = updated))
    }
    ...
    fun regenerateLastResponse() {
        ...
        val messages = sessions[sIdx].messages
    ...
    fun quoteMessage(messageIndex: Int) {
        ...
        val msg = sessions[sIdx].messages.getOrNull(messageIndex) ?: return
    ...
    private fun updateStep(sIdx: Int, msgIdx: Int, type: StepType, content: String, toolName: String? = null) {
        ...
        if (existingIdx != -1) {
            ...
            if (type == StepType.TOOL_CALL) {
                val isRunningMarker = content.startsWith("执行中:")
                val isLikelyArgs = !isRunningMarker && (content.trimStart().startsWith("{") || content.trimStart().startsWith("["))
                val isOutput = !isRunningMarker && !isLikelyArgs
                val err = if (isOutput && (content.startsWith("错误") || content.startsWith("工具执行失败"))) content else prev.error

                steps[existingIdx] = prev.copy(
                    content = prev.content,
                    input = if (isLikelyArgs) content else prev.input,
                    output = if (isOutput) content else prev.output,
                    error = err,
                    startedAt = prev.startedAt ?: now,
                    finishedAt = if (isOutput) now else prev.finishedAt,
                    isFinished = if (isOutput) true else prev.isFinished
                )
            } else {
            ...
            if (type == StepType.TOOL_CALL) {
                ...
                steps.add(
                    ChatStep(
                        ...
                        toolName = toolName,
                        isFinished = false,
                        input = if (isLikelyArgs) content else null,
                        output = if (!isRunningMarker && !isLikelyArgs) content else null,
                        error = null,
                        startedAt = now,
                        finishedAt = null
                    )
